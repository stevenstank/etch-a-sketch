...
pick_color.textContent = "Pick Color:"

const colorInput = document.createElement("input");
colorInput.setAttribute("type", "color");
colorInput.setAttribute("id", "colorPicker");
colorInput.setAttribute("value", "#ff0000ff")
...
const defaultSquareColor = '#3c3c3c';  
let currentDrawColor = '#ff0000ff';     
...
function draw(e) {
    if (e.target.classList.contains('square')){
        e.target.style.backgroundColor = currentDrawColor
    }
}
...
colorInput.addEventListener('input', function() {
    ...
    currentDrawColor = currentColor;
})

solid_color.addEventListener('click', function() {
 currentDrawColor = currentColor
})

erase_btn.addEventListener('click', function(){
    currentDrawColor = defaultSquareColor
})
...
pick_color.appendChild(colorInput)
